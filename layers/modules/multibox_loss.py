...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, priors, targets):
        ...
        priors = priors
        num = loc_data.size(0)  # batch size 大小 8
        num_priors = (priors.size(0))  # 预选框数量 1050
        ...
        for idx in range(num):
            ...
            # 把和每个prior box 有最大的IOU的ground truth box进行匹配，同时，编码包围框，返回匹配的索引，对应的置信度和位置
            #match(self.threshold, truths, defaults, self.variance, labels, landms, loc_t, conf_t, landm_t, idx)  # 匹配 ground truth
            match(self.threshold, truths, defaults, self.variance, labels, loc_t, conf_t, landm_t,idx)  # 匹配 ground truth
        if GPU:
            loc_t = loc_t.cuda()
            conf_t = conf_t.cuda()
        ...
        # pos1 = conf_t > zeros  # 匹配中所有的正样本mask, shape[b, M]
        ...
        pos_idx = pos.unsqueeze(pos.dim()).expand_as(loc_data)  # 正样本坐标存放的索引
